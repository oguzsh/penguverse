...
const STATES: AgentState[] = ['idle', 'working', 'thinking', 'sleeping', 'speaking', 'idle'];

let mockAgents: AgentStatus[] = [
  { id: 'emperor', name: 'Emperor', state: 'idle', task: null, energy: 1 },
  { id: 'pebble', name: 'Pebble', state: 'working', task: 'Writing code', energy: 0.8 },
];
...
function getMockData(): AgentStatus[] {
  ...
  if (stateIndex % 3 === 0) {
    for (const agent of mockAgents) {
      ...
      agent.task = agent.state === 'working' ? 'Writing code'
        : agent.state === 'thinking' ? 'Pondering...'
        : agent.state === 'speaking' ? 'Hey there!'
        : null;
    }
  }
  return [...mockAgents];
